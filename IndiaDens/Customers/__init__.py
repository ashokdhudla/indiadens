...
def login_required(f):
    ...
    def wrap(request, *args, **kwargs):
        if 'Customer' in request.session or 'User' in request.session:
          return f(request, *args, **kwargs)
        else:  
          if request.get_full_path().__contains__('/Customer'):
            return HttpResponseRedirect('/logout?login=customer')
          else:
